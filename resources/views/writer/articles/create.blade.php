...
@section('title', 'Buat Artikel Baru - Blog Cuaca')

@section('page-title')
    <h1>Buat Artikel Baru</h1>
    <p>Tulis dan bagikan informasi cuaca terbaru</p>
@endsection
...
@section('sidebar-menu')
    <ul class="sidebar-menu">
        ...
        <li class="sidebar-menu-item">
            <a href="{{ route('writer.create') }}" class="sidebar-menu-link active">
    ...
    <div style="background: rgba(15, 23, 42, 0.8); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: var(--radius-lg); padding: 24px;">
        <form method="POST" action="{{ route('writer.store') }}" enctype="multipart/form-data">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <input type="text" name="title" value="{{ old('title') }}" class="input-field" style="width: 100%; padding: 12px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: var(--radius-md); color: var(--text-primary);" required placeholder="Masukkan judul yang menarik">
            ...
            <div style="margin-bottom: 20px; display: flex; gap: 20px;">
                <div style="flex: 1;">
                    ...
                    <select name="category_id" style="width: 100%; padding: 12px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: var(--radius-md); color: var(--text-primary); appearance: none;" required>
                        <option value="" disabled selected style="background: #0F1729; color: white;">Pilih Kategori</option>
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id') == $category->id ? 'selected' : '' }} style="background: #0F1729; color: white;">{{ $category->name }}</option>
                        @endforeach
                    </select>
                    @error('category_id') <span style="color: #EF4444; font-size: 12px; display: block; margin-top: 4px;">{{ $message }}</span> @enderror
                </div>
                <div style="flex: 1;">
                    <label style="display: block; font-size: 13px; font-weight: 600; color: var(--text-secondary); margin-bottom: 8px; text-transform: uppercase;">Gambar Cover (Opsional)</label>
            ...
            <div style="margin-bottom: 24px;">
                <label style="display: block; font-size: 13px; font-weight: 600; color: var(--text-secondary); margin-bottom: 8px; text-transform: uppercase;">Konten Artikel (Minimal 50 Karakter)</label>
                <textarea name="content" rows="15" class="input-field" style="width: 100%; padding: 12px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: var(--radius-md); color: var(--text-primary); font-family: inherit;" required placeholder="Tuliskan isi artikel Anda di sini...">{{ old('content') }}</textarea>
                @error('content') <span style="color: #EF4444; font-size: 12px; display: block; margin-top: 4px;">{{ $message }}</span> @enderror
            </div>

            <div style="display: flex; gap: 12px; align-items: center;">
                <button type="submit" class="btn-sm btn-primary" style="padding: 12px 24px; font-size: 14px;">Simpan sebagai Draft</button>
                <span style="color: var(--text-muted); font-size: 12px;">Anda dapat mempublikasikannya nanti melalui menu Edit.</span>
            </div>
        </form>
    </div>
@endsection
