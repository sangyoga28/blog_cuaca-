@extends('layouts.app')

@section('title', 'Komentar Pengunjung - Blog Cuaca')

@section('page-title')
    <h1>Komentar Pengunjung</h1>
    <p>Lihat dan balas komentar pada artikel Anda</p>
@endsection

@section('sidebar-menu')
    <ul class="sidebar-menu">
        <li class="sidebar-menu-item">
            <a href="{{ route('writer.dashboard') }}" class="sidebar-menu-link">
                <svg viewBox="0 0 20 20" fill="currentColor"><path d="M3 1a1 1 0 000 2h1.22l.305 1.222a.997.997 0 00.01.042l1.358 5.43-.893.892C3.74 11.846 4.632 14 6.414 14H15a1 1 0 000-2H6.414l1-1H14a1 1 0 00.894-.553l3-6A1 1 0 0017 6H6.28l-.31-1.243A1 1 0 005 4H3z"/></svg>
                Dashboard
            </a>
        </li>
        <li class="sidebar-menu-item">
            <a href="{{ route('writer.articles') }}" class="sidebar-menu-link">
                <svg viewBox="0 0 20 20" fill="currentColor"><path d="M9 4.804C9 4.393 9.448 4 10 4s1 .393 1 .804v4.392h4.392c.411 0 .804.448.804 1 0 .552-.393 1-.804 1H11v4.392c0 .411-.448.804-1 .804s-1-.393-1-.804v-4.392H4.608c-.411 0-.804-.448-.804-1 0-.552.393-1 .804-1h4.392V4.804z"/></svg>
                Artikel
            </a>
        </li>
        <li class="sidebar-menu-item">
            <a href="{{ route('writer.create') }}" class="sidebar-menu-link">
                <svg viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"/></svg>
                Buat Artikel
            </a>
        </li>
        <li class="sidebar-menu-item">
            <a href="{{ route('writer.comments') }}" class="sidebar-menu-link active">
                <svg viewBox="0 0 20 20" fill="currentColor"><path d="M2 5a2 2 0 012-2h12a2 2 0 012 2v10a2 2 0 01-2 2H4a2 2 0 01-2-2V5z"/></svg>
                Komentar
            </a>
        </li>
    </ul>
@endsection

@section('content')
    <div style="background: rgba(15, 23, 42, 0.8); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: var(--radius-lg); padding: 20px;">
        <h3 style="margin-top: 0; color: var(--text-primary); margin-bottom: 20px;">Komentar Terbaru</h3>
        
        <div class="table-container">
            <table>
                <thead>
                    <tr>
                        <th>Pengunjung</th>
                        <th>Artikel</th>
                        <th>Komentar</th>
                        <th>Balasan Anda</th>
                        <th>Tanggal</th>
                        <th>Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($comments as $comment)
                        <tr>
                            <td>
                                <strong>{{ $comment->user->name }}</strong>
                            </td>
                            <td>
                                <a href="{{ route('article', $comment->post->slug) }}" target="_blank" style="color: var(--primary); text-decoration: none;">
                                    {{ Illuminate\Support\Str::limit($comment->post->title, 30) }}
                                </a>
                            </td>
                            <td>{{ $comment->content }}</td>
                            <td>
                                @if($comment->reply)
                                    <span style="color: #22C55E; font-size: 13px;">{{ $comment->reply }}</span>
                                @else
                                    <span style="color: var(--text-muted); font-size: 13px; font-style: italic;">Belum dibalas</span>
                                @endif
                            </td>
                            <td>{{ $comment->created_at->format('d M Y H:i') }}</td>
                            <td>
                                <form method="POST" action="{{ route('writer.reply', $comment->id) }}" style="display: flex; gap: 8px;">
                                    @csrf
                                    <input type="text" name="reply" placeholder="Tulis balasan..." value="{{ $comment->reply }}" style="padding: 6px 10px; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: var(--radius-sm); color: var(--text-primary); font-size: 12px; min-width: 150px;" required>
                                    <button type="submit" class="btn-sm btn-primary">Balas</button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" style="text-align: center; color: var(--text-muted);">Belum ada komentar di artikel Anda.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>

        @if($comments->hasPages())
            <div style="margin-top: 20px;">
                {{ $comments->links() }}
            </div>
        @endif
    </div>
@endsection
